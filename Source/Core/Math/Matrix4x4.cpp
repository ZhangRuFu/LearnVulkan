...
Matrix4x4& Matrix4x4::SetScale(const Vector3& inScale)
{
...
Matrix4x4& Matrix4x4::SetTranslate(const Vector3& inTrans)
{
...
Matrix4x4& Matrix4x4::AdjustDepthRange(float newNear, float newFar)
{
...
Quaternionf Matrix4x4::GetRotation() const
{
	throw std::exception("Not Implemented[need delete SIMD]");
	/* Assert(ValidTRS()); */

	/* delete simd */

	/*
	Matrix3x3f rotMat = Matrix3x3f(*this);
	rotMat = math::rotation(rotMat);

	Quaternionf result;
	MatrixToQuaternion(rotMat, result);
